...
def harc(pakli, kazamata_lista, file_helye, gyujtemeny_kartyai):
    ...
    if not pakli or not kazamata_lista:
        raise ValueError("Pakli vagy kazamata lista üres.")

    kor = 1
    akt_jatekos = pakli[0]
    akt_kazamata = kazamata_lista[0]
    kezdo_nev = getattr(akt_kazamata, "elnevezes", akt_kazamata.nev)

    with file_helye.open("w", encoding="utf-8") as f:
        f.write(f"harc kezdodik;{kezdo_nev}\n\n")

        # Első kijátszás
        f.write(f"{kor}.kor;kazamata;kijatszik;{akt_kazamata.nev};{akt_kazamata.sebzes};{akt_kazamata.eletero};{akt_kazamata.tipus}\n")
        f.write(f"{kor}.kor;jatekos;kijatszik;{akt_jatekos.nev};{akt_jatekos.sebzes};{akt_jatekos.eletero};{akt_jatekos.tipus}\n\n")

        while pakli and kazamata_lista:
            # Kazamata támad
            dmg_k = sebzes_szamitas(akt_kazamata.tipus, akt_jatekos.tipus, akt_kazamata.sebzes)
            akt_jatekos.eletero -= dmg_k
            if akt_jatekos.eletero < 0:
                akt_jatekos.eletero = 0
            f.write(f"{kor}.kor;kazamata;tamad;{akt_kazamata.nev};{dmg_k};{akt_jatekos.nev};{akt_jatekos.eletero}\n")

            # Ha a játékos meghal
            if akt_jatekos.eletero <= 0:
                pakli.pop(0)
                if not pakli:
                    f.write("jatekos vesztett\n")
                    break
                else:
                    # Következő játékos a következő körben lép be
                    kor += 1
                    akt_jatekos = pakli[0]
                    f.write(f"{kor}.kor;jatekos;kijatszik;{akt_jatekos.nev};{akt_jatekos.sebzes};{akt_jatekos.eletero};{akt_jatekos.tipus}\n\n")
                    continue
            ...
            # Játékos támad
            dmg_j = sebzes_szamitas(akt_jatekos.tipus, akt_kazamata.tipus, akt_jatekos.sebzes)
            akt_kazamata.eletero -= dmg_j
            if akt_kazamata.eletero < 0:
                akt_kazamata.eletero = 0
            f.write(f"{kor}.kor;jatekos;tamad;{akt_jatekos.nev};{dmg_j};{akt_kazamata.nev};{akt_kazamata.eletero}\n\n")

            # Ha a kazamata meghal
            if akt_kazamata.eletero <= 0:
                kazamata_lista.pop(0)
                if not kazamata_lista:
                    # Játékos nyert → nyeremény
                    if hasattr(akt_kazamata, "mitad"):
                        if akt_kazamata.mitad == "eletero":
                            akt_jatekos.eletero += 2
                            f.write(f"jatekos nyert;eletero;{akt_jatekos.nev}\n")
                        elif akt_kazamata.mitad == "sebzes":
                            akt_jatekos.sebzes += 1
                            f.write(f"jatekos nyert;sebzes;{akt_jatekos.nev}\n")
                        else:  # nagy kazamata
                            for k in gyujtemeny_kartyai.kartyak:
                                f.write(f"jatekos nyert;{k.nev}\n")
                                break
                    else:
                        f.write(f"jatekos nyert;eletero;{akt_jatekos.nev}\n")
                    break
                else:
                    # Következő kazamata csak a következő körben lép be
                    kor += 1
                    akt_kazamata = kazamata_lista[0]
                    f.write(f"{kor}.kor;kazamata;kijatszik;{akt_kazamata.nev};{akt_kazamata.sebzes};{akt_kazamata.eletero};{akt_kazamata.tipus}\n")
                    f.write(f"{kor}.kor;jatekos;kijatszik;{akt_jatekos.nev};{akt_jatekos.sebzes};{akt_jatekos.eletero};{akt_jatekos.tipus}\n\n")
